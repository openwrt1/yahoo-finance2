...
## 15. IPO 日历 (IPO Calendar)

抓取雅虎财经网页版的近期/未来美股 IPO (首次公开募股) 列表。

- **测试地址 (本周):** http://64.69.34.176:3007/calendar/ipo
- **测试地址 (指定日期):** http://64.69.34.176:3007/calendar/ipo?date=2024-03-01
- **接口路径:** `/calendar/ipo`
- **参数说明:**
  - `date` (可选): 查询指定日期所在周的 IPO 列表，格式为 `YYYY-MM-DD`。如果不传，默认查询当前周。
- **返回数据:**
  - `queryDate`: 查询的基准日期。
  - `count`: 查到的新股数量。
  - `data`: 包含 `symbol` (代码), `company` (公司名), `exchange` (交易所), `date` (预计上市日), `priceRange` (发行价区间), `shares` (发行股数) 的数组。
